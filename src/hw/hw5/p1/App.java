...
public class App {
    ...
    public static void main(String[] args) {
        ...
        list.stream()
                .filter(name-> name.getCategory().equals("전자기기류"))
                .forEach(System.out::println);

        //2.apple사가 전제품 10% 할인한다고 할때, 해당제품명과 할인된 가격을 출력하세요.
        list.stream()
                .filter(name -> name.getBrand().equals("apple"))
                .forEach(p -> {
                    int discounted = (int)( p.getPrice() * 0.9);
                    System.out.println(p.getProductName() + "- 할인된 가격:  " + discounted + "원");
                });
        ...
        List<Product> alist =  list.stream()
